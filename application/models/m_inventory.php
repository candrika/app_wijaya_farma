...
class m_inventory extends CI_Model {
	...
	function get($query=null,$id=null,$idunit=null,$inventory_class_id=null,$is_sellable=null,$is_purchasable=null,$business_id=null){
		$wer = null;
		...
		$sql = ("select a.product_id,a.product_name,a.product_no,a.product_description,a.cost_price,a.buy_price,a.wholesale_price,a.retail_price,a.weight,a.product_image,a.stock_available,a.stock_commited,a.stock_incoming,a.stock_max_online,a.status,a.available_on_pos,a.only_for_member,a.no_sku,a.no_barcode,a.no_supplier,a.is_sellable,a.is_purchasable,a.is_taxable,
			b.namecat,b.idinventorycat,c.brand_name,d.namesupplier,a.idsupplier,a.idunit,a.idbrand,a.product_type_id,a.member_price,a.retail_price as price,wholesale_price_member,retail_price_member,inventory_class_id,
			case
				when inventory_class_id = 1 then 'Barang'
				when inventory_class_id = 2 then 'Jasa'
				else 'Undefined'
			end as inventory_class_name,a.business_id,e.business_name,
			coa_sales_id,f.accname as coa_sales_name,
			coa_purchase_id,g.accname as coa_purchase_name,
			coa_tax_sales_id,
			coa_tax_purchase_id,is_consignment,consignment_base_price,consignment_owner_id,consignment_owner_type_id,
			case
				when consignment_owner_type_id = 1 then h.member_name
				when consignment_owner_type_id = 2 then i.namesupplier
				else 'Undefined'
			end as owner_name,a.product_unit_id,a.product_balance,a.coa_inventory_id,k.accname as coa_inventory_name,a.location_id,l.location_name
			from product a
			...
			left join business e ON a.business_id = e.business_id
			...
			left join product_location l on a.location_id=l.product_location_id and a.idunit = k.idunit
	...
	function get_stock($startdate,$enddate,$id_product){
		
		if($startdate!=null && $enddate!=null && $id_product!=null){
			$filter = " where b.idunit=".$this->user_data->idunit." and (datein between '".$this->date_filter($startdate)." 00:00:00 ' and '".$this->date_filter($enddate)." 23:59:59') and a.product_id=$id_product";
		}else if($startdate==null && $enddate==null && $id_product!=null){
			$filter = " where b.idunit=".$this->user_data->idunit." and a.product_id=$id_product";
		}else if($startdate!=null && $enddate!=null && $id_product==null){
			$filter = " where b.idunit=".$this->user_data->idunit." and (datein between '".$this->date_filter($startdate)." 00:00:00 ' and '".$this->date_filter($enddate)." 23:59:59')";
		}else{
			$filter = " where b.idunit=".$this->user_data->idunit;
		}

		$q = $this->db->query("select a.stock_history_id,a.product_id,a.type_adjustment,a.no_transaction,a.datein,a.notes,
							  a.datein,a.current_qty,a.trx_qty,a.new_qty,b.product_name
							  from stock_history a 
							  join product b ON b.product_id=a.product_id $filter and type_adjustment !=10 order by stock_history_id DESC");
		return $q;
	}
	...
	function stock_opname($idunit,$startdate=null,$enddate=null,$query=null){
		$where = "TRUE AND a.deleted=0 and (e.idunit is not null or f.idunit is not null and e.idunit=$idunit  and f.idunit=$idunit)";
		...
		$sql = "SELECT a.opname_number,a.stock_opname_id,sum(b.adjustment_stock) as adjustment_stock, 
				...
				e.firstname as userin_name,a.status,f.firstname as approved_name
				FROM stock_opname a
				...
				LEFT JOIN employee e ON e.user_id=a.userin and c.idunit=e.idunit
				LEFT JOIN  employee f ON f.user_id=a.approved_by and d.idunit=f.idunit
	...
	// function data_trans_stock_orgin($transfer_stock_id,$no_sku,$location_origin_id){
 ...
 //                                    A 
 //                                    INNER JOIN product b ON b.business_id = A.location_origin_id
 //                                    JOIN business C ON C.business_id = b.business_id
 //                                WHERE
 //                                    A.transfer_stock_id = $transfer_stock_id 
 //                                    and a.location_origin_id=$location_origin_id and b.no_sku='$no_sku'
 //                                GROUP BY
 //                                    A.current_qty,
 //                                    A.transfer_qty,
 //                                    b.stock_available,
