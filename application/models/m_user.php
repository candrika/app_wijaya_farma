...
class m_user extends CI_Model {
    ...
    function cekUser($id, $pass, $absen = false) {
        ...
        //1. sys_user 
        //2. ms_pegawai
        $q = $this->db->query("select a.*,c.group_name
                            from sys_user a
                            join sys_group c ON a.group_id = c.group_id
                            where (a.username = '".$id."' or email='".$id."') and a.password = '".$pass."' and a.deleted=0 and a.display is null");
        if ($q->num_rows() > 0) {
            $r = $q->row();       
            ...
            if($r->group_id==99)
            {
                ...
                $unit=null;
                    ...
                    //administrtor       
                    $qunit = $this->db->get_where('unit',array('idunit'=>12))->row();

                    $quunit = $this->db->get_where('userunit',array('user_id'=>$r->user_id))->row(); 
                    $qunit = $this->db->get_where('unit',array('idunit'=>$qunit->idunit))->row(); 
                    $unit = $qunit->namaunit;
                    $logo = $qunit->logo;
            ...
            $dataSession = array(
                ...
                'clientid' => $r->clientid,
                'username' => $r->username,
                'api_key' => base64_encode($r->username.':'.$r->password),
                'password' => $r->password,
                'realname' => $realname,
                'group_id' => $r->group_id,
                'logo' => $logo,
