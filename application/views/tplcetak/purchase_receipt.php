		...
		<div class="container">
			<div class="details clearfix">
				...
				<div class="data right" style="line-height: 20px;">
					<div class="title">No Penerimaan:&nbsp;<?=$data['d']->{'purchase_receipt_no'}?></div>
					<div class="date">
						Tanggal Pembelian: <?=$date_purchase?><br>
						Tanggal Penerimaan: <?=$receipt_date?><br>
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<thead>
					<tr>
						...
						<th class="unit">Qty Penerimaan</th>
				...
				<tbody>
					<?php
						$total_qty_penerimaan=0;
						$total_qty_sisa=0;
						$total_penerimaan=0;

						foreach ($data['d2'] as $key => $v) {
						# code...
						$total = $v->{'price'}*$v->{'qty_received'};

						$total_qty_penerimaan +=$v->{'qty_received'};
					...
					<tr>
						...
						<td class="unit"><?=$v->{'qty_received'}?></td>
						<td class="total"><?=$v->{'total_rest_qty'}?></td> 
						<td class="unit">Rp. <?=number_format($v->{'price'}*$v->{'qty_received'})?></td>
			...
			<div class="no-break">
				<table class="grand-total">
					<tbody>
						...
						<tr>
							<td class="desc" colspan="7"></td>
							<td class="unit">Total qty Penerimaan:</td>
							<td class="total"><?=number_format($total_qty_penerimaan)?></td>
						</tr>
						<tr>
							<td class="desc" colspan="7"></td>
							<td class="unit">Total Qty Sisa:</td>
						...
						<tr>
							...
							<?php
								if($data['d']->{'status_purchase_receipt'}*1==3){
							?>
							<td class="total">Rp. <?=number_format($total_penerimaan)?></td>
							<?php
								}else{
							?>
							<td class="total">Rp. <?=number_format($total_penerimaan)?></td>
