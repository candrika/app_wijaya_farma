...
?>
	<header class="clearfix">
		<div class="container">
			<figure>
				<img src="<?=base_url()?>/assets/images/<?=$this->session->userdata('logo')?>" width="100">
			</figure>
			<div class="company-address">
				<h2 class="title"><?=$data['company_name']?></h2>
				<p>
					<?=$company['alamat']?><br>
			...
			<?php
				if(isset($company['telp']) && isset($company['email'])){
			?>
			<div class="company-contact">
				<div class="phone left">
					...
					<?php
						if(isset($company['telp'])){
					?>
					<a href="<?=$company['telp']?>"><?=$company['telp']?></a>
					<?php
					}
					?>
		...
		<div class="container">
			<div class="details clearfix">
				<div class="client left">
					...
					?>
					<p class="name"><?=$data['d']->{'buyer_name'}?></p>
					<?php
					}else{
					?>
					<p class="name"><?=$data['d']->{'buyer_name'}?></p>
					...
					<?php
						if(!empty($data['email'])){
					?>
					<a href="mailto:john@example.com"><?=$data['email']?></a>
					<?php
						}
					?>
				</div>
				<div class="data right" style="line-height: 20px;">
					<div class="title">No Invoice: <?=$data['d']->{'noinvoice'}?></div>
					<div class="date">
						Tanggal Invoice: <?=$date_sales?><br>
						Tanggal Jatuh Tempo: <?=$due_date?><br>
						Term Payment: <?=$payment_term?><br>
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<thead>
					<tr>
						...
						<th class="qty">Qty</th>
						<th class="unit">Diskon</th>
						<!-- <th class="unit">Pajak</th> -->
				...
				<tbody>
					...
					<tr>
						...
						<td class="unit">Rp. <?=number_format($disc)?> (<?=$v->{'disc'}?>%)</td>
						<!-- <td class="total">Rp. <?=number_format($v->{'total_tax'})?> (<?=$v->{'rate'}?>%)</td> -->
						<td class="unit">Rp. <?=number_format($total)?></td>
					</tr>
					<?php
					}
					?>
				...
				<table class="grand-total">
					<tbody>
						...
						<tr>
							...
							<td class="total">Rp. <?=number_format($data['d']->{'disc'})?></td>
						</tr>
						<tr>
							<td class="desc" colspan="7"></td>
							<td class="unit">Biaya Kirim (+):</td>
							<td class="total">Rp. <?=number_format($data['d']->{'freight'})?></td>
						</tr>
						<tr>
							<td class="desc" colspan="7"></td>
							<td class="unit">Total:</td>
							<td class="total">Rp. <?=number_format($data['d']->{'total'})?></td>
						</tr>
						<?php
							if(($data['d3']->{'is_tax_ppn'} ==1  && $data['d3']->{'is_tax_pph23'} == 0) || ($data['d3']->{'is_tax_ppn'} ==0  && $data['d3']->{'is_tax_pph23'} == 1)){
						?>
						<tr>
							<?php
							if($data['d3']->{'is_tax_ppn'} == 1 && $data['d3']->{'is_tax_pph23'} == 0){
							?>
							<td class="desc" colspan="7"></td>
								<td class="unit">Pajak <?=$data['d3']->{'nametax'}?> <?=$data['d3']->{'coa_ppn_rate'}?>%:</td>
							<?php								
							}
							?>
							<?php
							if($data['d3']->{'is_tax_ppn'} == 0 && $data['d3']->{'is_tax_pph23'} == 1){
							?>
							<td class="desc" colspan="7"></td>
								<td class="unit">Pajak <?=$data['d3']->{'nametax'}?> <?=$data['d3']->{'coa_pph23_rate'}?>%:</td>
							<?php								
							}
							?>
							...
							if($data['d3']->{'is_tax_ppn'} == 1 && $data['d3']->{'is_tax_pph23'} == 0){
							?>
							 <td class="total">Rp. <?=number_format($data['d3']->{'total_tax'})?></td>
							<?php								
							}
							?>
							<?php
						...
						?>
						<?php
							if($data['d3']->{'is_tax_ppn'} ==1  && $data['d3']->{'is_tax_pph23'} == 1){
							?>
							<tr>
								<td class="desc" colspan="7"></td>
								<td class="unit">Pajak PPN <?=$data['d3']->{'coa_ppn_rate'}?>%:</td>
								<td class="total">Rp. <?=number_format(($data['d3']->{'coa_ppn_rate'}/100)*$data['d']->{'total'})?></td>
							</tr>
							<tr>	
								<td class="desc" colspan="7"></td>
								<td class="unit">Pajak Pph 23 <?=$data['d3']->{'coa_pph23_rate'}?>%:</td>
								<td class="total">Rp. <?="(".number_format(($data['d3']->{'coa_pph23_rate'}/100)*$data['d']->{'total'}).")"?></td>
							</tr>
							<?php								
							}
							?>
						<tr>
							<td class="desc" colspan="7"></td>
							<td style="font-weight:600;font-size: 1.18181818181818em;">GRAND TOTAL:</td>
							<td style="font-weight:600;font-size: 1.18181818181818em;">Rp. <?=number_format($data['d3']->{'grand_total'})?></td>

						</tr>
						<?php
							if($data['d']->{'paidtoday'}*1!=0){
						?>
						<tr>
							<td class="desc" colspan="7"></td>
							<td style="font-size: 1.18181818181818em;">Jumlah Terbayar:</td>
							<td style="font-size: 1.18181818181818em;">Rp. <?=number_format($data['d']->{'paidtoday'})?></td>
						</tr>
						<?php
							}
						?>
						<?php
							if($data['d']->{'unpaid_amount'}*1!=0){
						?>
						<tr>
							<td class="desc" colspan="7"></td>
							<td class="unit">Jumlah Yang Harus Dibayar:</td>
							<td class="total">Rp. <?=number_format($data['d']->{'unpaid_amount'})?></td>
						</tr>
						<?php
							}
						?>
